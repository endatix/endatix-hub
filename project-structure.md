...
A page that _reports_ on a domain (admin Environment, health views) imports that domain's
...
- Client tables update the URL via `useListUrlState` (single debounced field) or `useTableFiltersUrlState` (2+ debounced fields, one coalesced commit — see below), plus `Select` filters calling `updateUrl` directly, `PagedTableFooter` / `PagedListFooter`, and receive unresolved promises from the server page.
- **Paged list wiring (one pattern, issue #1011).** The server page parses the request, starts the promise, and passes `listKey={listQueryKey(listRequest)}` (`lib/list-page/list-query-key`). Never hand-write a per-list key: `listQueryKey` covers every field the parser emits, so new filters and sort fields can't be missed.
  - A **client shell** owns `useListUrlState` once and renders the toolbar **outside** `PagedListFrame` (`components/table`) and the grid **inside** it. The grid calls `use(promise)` and gets `updateUrl` / `isPending` as props — it never calls a URL hook itself. Reference: `features/platform-admin/list-tenants/ui/tenants-list.tsx`. Card-style lists (`UsersList`, `RolesList`, `PlatformAdminsList`) use `PagedCardList`, which puts the toolbar in the card header and the frame around the rows.
  - When the toolbar and grid are **server-composed** (other server content between them, e.g. folder cards on `/forms`), wrap them in `PagedListUrlProvider` and read `usePagedListUrl()` in both. Same rule: toolbar outside the frame.
  - Why the key: without it, `use(promise)` keeps the first page after `router.replace` until a full reload. Why the toolbar stays outside: a key change remounts everything inside the frame and shows the fallback, so a search input inside it loses focus and typed characters on every debounced commit. A page change shows the skeleton; that is intended.
  - **Row identity.** Every TanStack grid sets `getRowId` to the entity id. Index-keyed rows let stateful cells (the submissions status dropdown, answers) keep the previous page's values — that was the submissions half of #1011.
  - **Submissions** key a server section: `PagedListFrame` wraps the async `SubmissionListSection` (it keeps the post-fetch `redirect()` on the server), and `SubmissionsWithFilters` hands `isPending` to its `DataTable` for skeleton rows while the next page is in flight.
  - Data-list **items** keep `Suspense` inside the client details section.
  - **Height while loading.** Whoever renders the pending skeleton holds the list at its last rendered height with `useHeldHeight(isPending)`: `PagedListFrame` for card lists and forms, `DataTableGrid` and the submissions `DataTable` for their row area (the footer sits below it). The skeleton can't shrink the page or move the pager while the next page loads. When rows render the list takes its natural height, so a filter with one result shows one row, every time, and a shorter last page is shorter. **One loading look:** while a page, filter, or sort change is in flight (`isPending` from the shell's `useListUrlState`), the rows become skeleton rows and everything else stays mounted — no toolbar "Updating…" text, no dimming. Table lists pass `isPending` to `DataTableGrid` (or the submissions `DataTable`), which keeps the column header (and its filters) and renders `DataTableSkeletonRows`. Card lists pass it to `PagedCardList`, and `PagedListFrame` swaps in its fallback; under `PagedListUrlProvider` (forms) the frame picks it up on its own. An empty list that is reloading shows skeleton rows too (`pendingRowCount`), not its empty state. Every frame fallback is a `*Skeleton` component. Spinners are for action buttons (grant, save, delete), never for list rows. The conformance test enforces the fallback names, `isPending` on every `DataTableGrid`, and no "Updating…" text.
  - **Tests, in three layers.** (1) `__tests__/app/paged-list-conformance.test.ts` fails the build when a TanStack grid lacks `getRowId`, a paged page skips `listQueryKey` without an allow-listed reason, or a key is hand-rolled. (2) `__tests__/pages/paged-list-shells.test.tsx` renders each shell, changes `listKey`, and checks new rows appear while the search input keeps its value and focus — add a new shell to its table. (3) `e2e/tests/lists/list-paging.spec.ts` pages every list in a real browser and compares the rows with a fresh render of the same URL — add a new list to its `lists` array.
- Shared helpers live in `lib/list-page/` (`parse-paged-search-params`, `list-query-key`, `table-return-to`). URL writers and the frame (`useListUrlState`, `useTableFiltersUrlState`, `PagedListUrlProvider`, `PagedListFrame`, `PagedCardList`) live in `components/table/`.
...
- Stream the paged promise from the App Router page into the client table (`use()` inside `PagedListFrame` with `listQueryKey`). Footer lives **inside** `DataTableSurface`. Items keep `Suspense` inside the details client section.
