...
/**
 * Creator chrome. Two depths, the same rule in both palettes — recessed
 * `--content-canvas` (working area, input fills), raised `--card` (chrome panels,
 * question cards). Region-by-token table: `hub/DESIGN.md` §9.
 *
 * Not `--background` for recessed — same `#fff` as `--card` in light, so inputs
 * lost their fill (endatix-hub#954) — and not `--content-canvas` for raised,
 * which flattens Creator into one colour. Leaving either unmapped parks it on
 * SurveyJS's neutral grey ramp, which reads warm grey against the Hub navy.
 */
