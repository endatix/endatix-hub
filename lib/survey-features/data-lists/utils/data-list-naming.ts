 ...
 */
export function isGenericChoiceQuestionTitle(
  plainTitle: string,
  questionType?: string,
): boolean {
  const normalizedTitle = plainTitle.trim().toLowerCase();
  if (!normalizedTitle) {
    return true;
  }
  if (!questionType) {
...
export function getQuestionDataListName(
  question: { title?: unknown; name: string; type?: string },
  ...
  const useTitle =
    fromTitle.length > 0 &&
    !isGenericChoiceQuestionTitle(fromTitle, question.type);
  const baseSource =
    (useTitle ? fromTitle : "") ||
    (fromName.length > 0 ? fromName : "Data list");
