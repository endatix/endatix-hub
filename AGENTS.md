...
- PDF theme (derive-only): `mapSurveyThemeToPdfTheme` reads SurveyJS `ITheme.cssVariables` (`--sjs2-*` / `--sjs-*`). No `Theme.cs` column and no nested `pdf` overlay in v1. Missing/invalid theme → `DEFAULT_PDF_THEME` (Endatix Hub light literals from `hubTokenFallbacks`, not SurveyJS DefaultLight green). Theme JSON comes from `formDefinition.themeModel` or `GET /forms/{id}/definition`. Hub export passes the session JWT; the share-link `/export-pdf` path is anonymous.
- How to compare Endatix default vs assigned form theme (same surfaces: **Download PDF** on submission details, **Export PDF** share-link from the share dialog):
  1. Form A: Theme tab unassigned / catalog **default** (`themeId` 0). Fill a submission. Export from details, then create an Export PDF share link and open it. Header bar + properties card should be Hub navy (`#0054d1`) on white.
  2. Form B: assign a tenant theme with a distinct brand (e.g. Sharp / a green `--sjs-primary-backcolor`). Same two export paths. Accent bar and property card should follow that theme, not Hub navy.
  3. Both paths must match each other for a given form. If they diverge, the Hub route is using session `GetActive` while the token route failed anonymous `GetActive` (falls back to default).
- PDF look-and-feel (not a pixel match): canvas / card / editor / brand / text / muted / border / radius / print-clamped font size. Brochure uses **Roboto** only.
- PDF AcroForm widgets (`Checkbox` / `Select` / `List` / `TextInput` / `FieldSet`) only when `fillable` is true on `renderSubmissionPdf` (default off). Brochure export is `Text` only — widgets on every question stall PDFKit `toBlob`. Matrix cells are always `Text`.
- PDF matrix: `PdfMatrixTable` + `computeMatrixDataColumnWidth` (even A4 split, stacked fallback). Cells: `row.isEmpty` / `getCellByColumn`, never `question.value[row.value]`.
...
- **[`.nvmrc`](.nvmrc) is the only place the Node version is written.** CI reads it via `node-version-file`, the Dockerfile repeats just the major, and `engines.node` is the supported range (LTS lines only - 22 and 24; `lib/hosting/check-node-version.ts` warns at startup when the host is outside it). To move Node, edit `.nvmrc` and, only when the supported _range_ changes, `engines.node` + the Dockerfile major. `__tests__/pnpm-toolchain.test.ts` fails on any drift, and [`dependabot.yml`](.github/dependabot.yml) is told not to raise the image major on its own.
...
- `?tab=` map: `creator/tab-url.ts`. Sync with [`useCreatorTabUrl`](lib/survey-features/survey-design/ui/use-creator-tab-url.ts) (`history.replaceState`). Lists keep `useUrlSearchParamsUpdater` so they refetch (h939). It runs before the hooks declared after it, so a deep link activates the plugin before they subscribe — wire tab work from `creator.activeTab` at mount too, never from `activate`/`onActiveTabChanged` alone.
...
- Theme hydration: `hydrateThemeTab` in [`use-theme-management.hook.ts`](features/themes/manage-theme-editor/use-theme-management.hook.ts). Hub dirty is `isThemeDirty`; Creator's save signal is `hasPendingThemeChanges` — do not clear the latter while the former is true. `creator.theme =` (`applyTheme`) skips the Theme tab plugin while that tab is active, so a theme that resolves after the tab opened must go in via `themeEditor.themeModel.setTheme`, inside `hydrateThemeTab`.
- Theme Editor `themeName` is lazy-paged: [`bind-theme-catalog-lazy-choices.ts`](features/themes/manage-theme-editor/bind-theme-catalog-lazy-choices.ts). Never drain the catalog at Creator init — the assigned theme is `GET /themes/{id}`. A loaded page only _registers_ themes (`addTheme`); re-applying the assigned one would discard the edits in progress. The last page must report the item count accumulated so far, or the chooser keeps a "Loading…" row. Chooser search stays off until `GET /themes` accepts a name filter. `activate()` rebuilds `propertyGrid.survey` every time, so a binding must supersede the previous one instead of stacking wrappers on the long-lived plugin.
- SurveyJS lazy-dropdown quirks are shared, not per-feature: skip/take math in [`choices-lazy-load-page.ts`](lib/survey-features/infrastructure/choices-lazy-load-page.ts), `DropdownListModel` internals (dedupe, buffer reset, loading-row observer) in [`choices-lazy-load-dropdown.ts`](lib/survey-features/infrastructure/choices-lazy-load-dropdown.ts). `setItems` **appends**, so dedupe before handing it a page. Poke vendor privates only from those two files — they are the upgrade checklist for `scripts/upgrade-surveyjs.mjs`.
