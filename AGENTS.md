...
- Theme Editor `themeName` is lazy-paged: [`bind-theme-catalog-lazy-choices.ts`](features/themes/manage-theme-editor/bind-theme-catalog-lazy-choices.ts). Never drain the catalog at Creator init — the assigned theme is `GET /themes/{id}`. A loaded page only *registers* themes (`addTheme`); re-applying the assigned one would discard the edits in progress. The last page must report the item count accumulated so far, or the chooser keeps a "Loading…" row — SurveyJS skip/take quirks live in [`choices-lazy-load-page.ts`](lib/survey-features/infrastructure/choices-lazy-load-page.ts) (shared with the data-list catalog). Chooser search stays off until `GET /themes` accepts a name filter.
