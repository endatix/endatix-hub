...
- **Do not test embed by opening `/embed/{formId}` alone** — that skips `embed.js`. Dev/agent playground: WebHost `GET /dev/embed-host?formId=` (Development; Hub `http://localhost:3000` by default). Playwright: `openEmbedHost` + `E2E_EMBED_HOST_URL` (API origin). Public-form data-collection features: check **share and embed**.
