...
- Submission label locale: one picker, the metadata card Language cell (`label-language.tsx`, DESIGN.md §6 “View-only choices”). Details page and `@header` do not share React context, so `SubmissionDetailsProvider` publishes `{ catalogLocales, displayCatalogLocale }` to `submission-display-locale.store.ts` while a model is loaded and clears it on unmount. The header derives one `pdfLocale` (multi-language surveys only) for Export PDF and the PDF share link. PDF locale query + policy live in `features/pdf-export/submission/pdf-locale.ts` (`PDF_LOCALE_PARAM`, `parsePdfLocaleQuery`, `resolvePdfLocale`): legacy `?defaultLocale=true` wins, unknown codes fall back to the survey default. Routes parse, they do not read locale params themselves.
