...
- **Height is reported, never pinned.** `EmbedHeightReporter` posts `document.body.scrollHeight` on every mutation/resize; the host clamps it. Do not suppress reporting around submit — the complete page is shorter than the form it replaces, so a suppressed shrink leaves the thank-you page below the iframe fold (h947).
- **No `vh`/`dvh` under `.embedShell`** (`survey-component.module.css`) — inside the iframe those resolve against the height we ourselves reported, so each resize message shrinks the next one. Viewport units belong in the standalone `.layoutFullHeight` rules; `features/public-form/ui/__tests__/survey-component-embed-css.test.ts` pins this.
- **Scroll signal.** `endatix:scroll` is smooth by default; `onCompleting` sends `behavior: "instant"` so the host is already at the iframe top before the form collapses. Hosts on an older `embed.js` ignore `behavior` and keep smooth.
