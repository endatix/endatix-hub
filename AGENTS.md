...
- Submission PDF: `features/pdf-export/`. Public token + Hub API routes both call `renderSubmissionPdf`. Bound by server env `PDF_RENDER_TIMEOUT_SECONDS` (default 40, never `NEXT_PUBLIC_`). Public failures: `/export-error?code=&ref=`. User-authored `<Text>`: `pdfPlainText` (`lib/utils/pdf-plain-text.ts`), not `sanitizeInline`. Helper lives in `lib/` because `lib/questions/*pdf-answer*` needs it.
