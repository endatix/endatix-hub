...
- Theme Editor `themeName` is lazy-paged (`Themes.list` + `choicesLazyLoadEnabled`). Do not `listAll`/`addTheme` the catalog at Creator init. Assigned theme is `GET /themes/{id}`. Shared skip/take math: [`choices-lazy-load-page.ts`](lib/survey-features/infrastructure/choices-lazy-load-page.ts) (also data-list catalog).
