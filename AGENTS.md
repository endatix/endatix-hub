...
- Submission PDF: `features/pdf-export/`. Public token + Hub API routes both call `renderSubmissionPdf`. Bound by server env `PDF_RENDER_TIMEOUT_SECONDS` (default 40, never `NEXT_PUBLIC_`). Public failures: `/export-error?code=&ref=`. Matrix grids: `PdfMatrixTable` + `computeMatrixDataColumnWidth` (even A4 split, stacked fallback). Read matrixdropdown/dynamic cells via `row.isEmpty` / `getCellByColumn`, never `question.value[row.value]` (`row.value` is the answer object). User-authored `<Text>`: `pdfPlainText` (`lib/utils/pdf-plain-text.ts`) — HTML strip + entity decode via `htmlSanitizer.toPlainText`, plus literal `\\n`. Not `sanitizeInline`. Helper lives in `lib/` because `lib/questions/*pdf-answer*` needs it. Hub UI labels use `htmlSanitizer.toPlainText` only.
