...
- **Thank-you in embed (h947).** Do not size `.embedShell .sd-completedpage` with `vh`/`dvh` (iframe height is what we report — feedback loop). Page change: `endatix:scroll` (smooth). Completing: `behavior: "instant"` from `onCompleting`. After `freeze` around complete/error, `embedHeightReporting.resume()` so height reports again (mutations while frozen do not re-fire).
