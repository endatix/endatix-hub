 ...
 */
export function PostHogUserIdentity({ session }: PostHogUserIdentityProps): null {
  ...
  useEffect(() => {
    ...
    const getOrCreateAnonymousId = () => {
      if (typeof window !== 'undefined') {
        ...
        const newId = `anon_${crypto.randomUUID ? crypto.randomUUID() : Math.random().toString(36).substring(2, 15)}`;
