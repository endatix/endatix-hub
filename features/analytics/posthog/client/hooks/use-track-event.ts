...
import { ensureReady, isPostHogInitialized, captureException } from "../client";
...
export const useTrackEvent = (
   ...
   */
  const trackEventFn = useCallback<TrackEventFunction>(
  ...
  const trackException = useCallback<TrackExceptionFunction>(
    (error, properties = {}) => {
      captureException(error, properties, config, clientOptions);
    },
    [config, clientOptions],
  );
