...
describe('FormTokenCookieStore', () => {
  ...
  function createStore() {
    return new FormTokenCookieStore(mockCookieStore as unknown as ReadonlyRequestCookies);
  }
  ...
  describe('initialization', () => {
    ...
    it('throws error when cookie name environment variable is missing', () => {
      process.env.NEXT_FORMS_COOKIE_NAME = '';
      expect(() => createStore()).toThrow('NEXT_FORMS_COOKIE_NAME environment variable is not set');
    });
    ...
    it('throws error when cookie duration environment variable is invalid', () => {
      ...
      expect(() => createStore()).toThrow('NEXT_FORMS_COOKIE_DURATION_DAYS environment variable is not set or invalid');
    });
  });
  ...
  describe('setToken', () => {
    it('adds new token when cookie exists with other tokens', () => {
      ...
      // Act
      const result = store.setToken({ formId: TEST_FORM_ID, token: TEST_TOKEN });
